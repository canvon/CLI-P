...
# Load this only after Qt has been initialized, or the `import cv2` (OpenCV2)
# may lead to weird errors... (See comment in `load_image.py`.)
load_image = None
def load_load_image():
    global load_image
    load_image = __import__('load_image').load_image  # Translates to: from load_image import load_image
...
class ThumbnailGenerator(QObject):
    ...
    def __init__(self, *, pool, poolProcessCount, largeSize=180, smallSize=24, parent=None):
        ...
        self.pool = pool
        self.poolProcessCount = poolProcessCount
        self.largeSize = largeSize
        self.smallSize = smallSize
        self.localWaiting = collections.deque()
        self.requests = collections.deque()
        self._timer = QTimer(self)
        self._timer.timeout.connect(self.doWork)
    ...
    def doWork(self):
        ...
        while i < len(self.requests):
            ...
            if not request.poolResult.ready():
                i += 1
                continue
            fix_idx = request.searchResult.fix_idx
            tfn = request.searchResult.tfn
            try:
                dataStreams = request.poolResult.get()
                if dataStreams is None or len(dataStreams) != 2:
                    raise RuntimeError(f"Unexpected pool result data {dataStreams!r}")
                largeThumb, smallThumb = map(unwrap_image, dataStreams)
                self.largeThumbnailReady.emit(fix_idx, tfn, largeThumb)
                self.smallThumbnailReady.emit(fix_idx, tfn, smallThumb)
            except Exception as ex:
                self.logMessage.emit(f"Got exception {type(ex).__name__} loading thumbnail for fix_idx={fix_idx}, tfn={tfn!r}: {ex}")
            finally:
        ...
        while len(self.localWaiting) > 0 and len(self.requests) < self.poolProcessCount:
            ...
            request.poolResult = self.pool.apply_async(load_image_thumbnails, (
                request.searchResult.tfn,
                [QSize(size, size) for size in [self.largeSize, self.smallSize]]
            ))
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *, pool, poolProcessCount, parent=None):
        ...
        self.pool = pool
        self.poolProcessCount = poolProcessCount
        self.thumbnailGenerator = ThumbnailGenerator(pool=pool, poolProcessCount=poolProcessCount, parent=self)
        self.search = None
    ...
    class SearchResultsModel(QStandardItemModel):
        ...
        def data(self, index, role):
            ...
            if result is None:
                return None
            self._mainWindow.thumbnailGenerator.requestThumbnail(result)
            return None
    ...
    def createSearchResultsModel(self):
        ...
        largeSize = self.thumbnailGenerator.largeSize
    ...
    def clearSearchResultsModel(self):
        ...
        self.thumbnailGenerator.cancelWaiting()
...
if __name__ == '__main__':
    ...
    processCount = os.cpu_count() or 1
    with multiprocessing.Pool(processes=processCount) as p:
        app = QApplication(sys.argv)
        app.setApplicationName("CLI-P GUI")
        win = MainWindow(pool=p, poolProcessCount=processCount)
        win.show()
        ret = app.exec_()
    sys.exit(ret)
